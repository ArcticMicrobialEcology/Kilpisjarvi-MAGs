...
# Read metadata
metadata <- read_delim("~/Scripts/Kilpisjarvi-MAGs/sample_metadata.tsv", delim = "\t") %>%
...
phylum <- read_delim("02_METAXA/summary_level_2.txt", delim = "\t") %>%                # Read data
...
genus <- read_delim("02_METAXA/summary_level_6.txt", delim = "\t") %>%                 # Read data
...
loadKEGG("PATH_TO_KEGG_DB")

## Read KEGG annotation
kegg <- lapply(SAMPLES, function(SAMPLE) {
  readBlast(paste("02_KEGG/", SAMPLE, ".txt", sep = ""))
}) %>%
  set_names(SAMPLES)
